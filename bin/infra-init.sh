...
if [ "$HOST" == "pincer" ]; then
  ...
  mkdir -p $R/go/efi
  echo 'LABEL=EFI /go/efi auto user,uid=501,gid=27,fmask=0144,dmask=0044,noexec,nosuid,nodev,x-systemd.automount,x-systemd.idle-timeout=3min 0 2' >> $R/etc/fstab
